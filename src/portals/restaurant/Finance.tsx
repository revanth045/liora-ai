  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-page-slide pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-stone-800 text-white p-8 rounded-[2rem] shadow-xl relative overflow-hidden group card-lift animate-slide-up stagger-1">
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-cream-200 shadow-sm flex flex-col justify-between card-lift animate-slide-up stagger-2">
          <div>
          ...
          <div className="mt-6">
            <div className="w-full bg-cream-50 rounded-full h-2.5 overflow-hidden shadow-inner">
              <div className="bg-orange-500 h-full rounded-full shadow-sm bar-fill" style={{ width: '65%' }}></div>
            </div>
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-cream-200 shadow-sm flex flex-col justify-between card-lift animate-slide-up stagger-3">
          <div>
          ...
          <button className="mt-6 text-[10px] font-bold uppercase tracking-widest text-stone-800 bg-cream-50 px-4 py-2.5 rounded-xl hover:bg-cream-200/60 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 active:scale-95 border border-cream-200/50 shadow-sm">
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-cream-200 shadow-sm">
          ...
          <div className="space-y-6">
            {[
              { label: 'Dine-In',          val: '65%', amt: '$27,878', color: 'bg-stone-800' },
              { label: 'Online / Takeout', val: '25%', amt: '$10,722', color: 'bg-amber-500' },
              { label: 'Bar / Alcohol',    val: '10%', amt: '$4,290',  color: 'bg-stone-400' },
            ].map((item, i) => (
              <div key={i} className="group">
                <div className="flex justify-between items-end text-sm mb-2">
                  <span className="font-bold text-stone-800 group-hover:text-stone-900 transition-colors">{item.label}</span>
                  <span className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">{item.amt} <span className="opacity-50">• {item.val}</span></span>
                </div>
                <div className="w-full bg-cream-50 rounded-full h-4 shadow-inner overflow-hidden">
                  <div
                    className={`${item.color} h-full rounded-full shadow-sm bar-fill group-hover:brightness-110 transition-all`}
        ...
        <div className="bg-white rounded-[2rem] border border-cream-200 shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="divide-y divide-cream-200 flex-1">
            {[
              ...
              <div key={i} className="p-5 flex justify-between items-center hover:bg-cream-50/60 transition-all duration-150 group cursor-default">
                <div className="flex items-center gap-4">
                  <div className="p-2.5 bg-cream-50 rounded-xl text-stone-800 group-hover:bg-cream-100 group-hover:scale-105 transition-all duration-150">
                    <Icon name="sync" size={18} />
                  </div>
                  <div>
                    <p className="font-bold text-stone-800 text-sm">Weekly Settlement</p>
                    <p className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">{tx.date} • {tx.bank}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-bold text-stone-800 text-lg group-hover:text-stone-900">{tx.amount}</p>
