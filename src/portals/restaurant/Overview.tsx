...
export default function RestoOverview({ restaurant }: { restaurant: DemoRestaurant }) {
  ...
  const STATUS_COLOR: Record<string, string> = { pending: 'bg-amber-100 text-amber-600', order: 'bg-green-100 text-green-600', service: 'bg-blue-100 text-blue-600', done: 'bg-stone-100 text-stone-600' };
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {METRICS.map((metric, i) => (
          <div key={i} className="bg-white p-5 rounded-3xl border border-cream-200 shadow-sm flex items-start justify-between group hover:shadow-md transition-shadow">
            <div>
              <p className="text-[10px] font-bold text-stone-600 uppercase tracking-widest mb-1">{metric.label}</p>
              <h3 className="text-2xl font-lora font-bold text-stone-800">{metric.value}</h3>
              <p className="text-[10px] text-stone-600 mt-1">{metric.sub}</p>
            </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-[2rem] border border-cream-200 shadow-sm overflow-hidden flex flex-col h-[500px]">
          <div className="p-6 border-b border-cream-200 flex justify-between items-center bg-cream-50/50">
            ...
            <span className="text-[9px] font-bold text-stone-600 uppercase tracking-widest bg-stone-100 px-2.5 py-1 rounded-full">Auto-refreshes · 5s</span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            {recentFeed.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-stone-600 py-8">
                ...
                <p className="font-bold text-stone-600">No orders yet today</p>
              ...
              <div key={item.id} className="flex items-center gap-4 p-4 hover:bg-cream-50/30 rounded-2xl transition-all border border-transparent hover:border-cream-200">
                <div className={`p-3 rounded-xl shadow-sm ${STATUS_COLOR[item.type] ?? 'bg-stone-100 text-stone-600'}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-0.5">
                    ...
                    <span className="text-[10px] font-bold text-stone-600 uppercase bg-cream-100/50 px-2 py-0.5 rounded-full">{item.time}</span>
                  </div>
                  <p className="text-xs text-stone-600 font-medium truncate">{item.action}</p>
                  {item.notes && (
                ...
                <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-1 rounded-full border ${
                  ...
                  item.status === 'delivered' ? 'bg-stone-50 text-stone-600 border-stone-200' :
            ...
            {recentFeed.length > 0 && (
              <div className="opacity-40 text-center text-[10px] font-bold uppercase tracking-[0.2em] text-stone-600 py-4">
          ...
          <div className="bg-white rounded-[2rem] border border-cream-200 shadow-sm overflow-hidden">
            ...
            {ALERTS.length === 0 ? (
              <div className="p-6 text-center text-stone-600">
                ...
                <p className="text-sm font-bold text-stone-600">All clear!</p>
              ...
              <div className="divide-y divide-cream-200">
                {ALERTS.map((alert) => (
                  <div key={alert.id} className={`p-5 border-l-4 transition-colors relative ${
                    ...
                    {alert.isTableAlert && (
                       <button onClick={() => handleDismissAlert(alert.id)} className="absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center bg-white border border-stone-200 text-stone-600 hover:text-green-600 hover:border-green-300 transition-colors shadow-sm">
            ...
            {[
              ...
              <div key={i} className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm text-stone-600">
                  <Icon name={s.icon} size={16} className={s.warn ? 'text-red-500' : 'text-stone-600'} />
                  {s.label}
                </div>
