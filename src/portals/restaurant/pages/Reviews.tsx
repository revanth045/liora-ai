    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-1 bg-white border border-cream-200 rounded-2xl p-6 flex flex-col items-center justify-center shadow-sm">
          ...
          <p className="text-sm text-stone-600 mt-2">{data.totalCount} reviews</p>
        </div>
        <div className="md:col-span-2 bg-white border border-cream-200 rounded-2xl p-6 shadow-sm">
          ...
          {data.distribution.map(({ star, count }) => (
            <div key={star} className="flex items-center gap-3 mb-1.5">
              <button onClick={() => setFilterRating(filterRating === star ? 0 : star)} className="flex items-center gap-1 w-16 text-xs text-stone-600 hover:text-brand-400 transition-colors">
      ...
      <div className="space-y-4">
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-stone-600">{timeAgo(review.createdAt)}</p>
                </div>
              </div>
              ...
              replyId === review.id ? (
                <div className="mt-3 space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => { setReplyId(null); setReplyText(''); }} className="text-xs px-4 py-2 text-stone-600 hover:text-stone-700 bg-cream-100 rounded-lg">Cancel</button>
                  </div>
                </div>
              ) : (
                <button onClick={() => { setReplyId(review.id); setReplyText(''); }} className="mt-3 flex items-center gap-1.5 text-xs text-stone-600 hover:text-brand-400 transition-colors font-medium">
        ...
        {filtered.length === 0 && <div className="text-center py-12 text-stone-600">No reviews match the selected filter.</div>}
      </div>
    </div>
  );
}
