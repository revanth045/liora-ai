...
const EMPTY_FORM: SpecialForm = {
  name: '', description: '', priceCents: 2500, category: 'daily_special', chefNote: '', imageEmoji: '  ΩÔ∏è', isAvailable: true,
};

const EMOJI_OPTIONS = ['  ΩÔ∏è','   ',' •©','   ',' ´‚Ä¢',' • ','  ±',' ¶ ',' ß ',' • ','  ø','   '];
...
export default function RestoChefSpecials({ restaurant }: { restaurant: DemoRestaurant }) {
  ...
  const openEdit = (s: DemoChefSpecial) => {
    setEditId(s.id);
    setForm({ name: s.name, description: s.description, priceCents: s.priceCents, category: s.category, chefNote: s.chefNote || '', imageEmoji: s.imageEmoji || '  ΩÔ∏è', isAvailable: s.isAvailable });
    setShowForm(true);
  };
                  ...
                  <div className="p-6">
                    ...
                    {special.chefNote && (
                      <div className="flex items-start gap-2 bg-amber-50 border border-amber-100 rounded-2xl px-4 py-3 mb-4">
                        <span className="text-lg flex-shrink-0">  ®‚ ç  ≥</span>
