...
export default function ChefGPT({ restaurant }: { restaurant: DemoRestaurant }) {
  const [messages, setMessages] = useState<Msg[]>([
    {
      role: 'chef',
      content: `Welcome, Chef! I'm Chef GPT â ” your AI culinary assistant for ${restaurant.name}.\n\nAsk me anything: recipes, techniques, ratios, substitutions, temperatures, or plating ideas. What are we cooking today? ð  ¨â  ð  ³`,
    },
  ]);
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5 bg-gradient-to-b from-amber-50/30 to-white">
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm text-lg ${
                ...
                {msg.role === 'chef' ? 'ð  ¨â  ð  ³' : <Icon name="person" size={18} />}
              </div>
          ...
          {isTyping && (
            <div className="flex gap-3 animate-pulse">
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-lg shadow-sm">ð  ¨â  ð  ³</div>
