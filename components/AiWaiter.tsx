...
export const AiWaiter = () => {
    ...
    const QUICK_ACTION_CONFIG: Record<string, { icon: string; alertMessage: string; confirmation: string }> = {
        'Call Waiter':    { icon: 'ð   ï¸', alertMessage: `Table ${session.tableNumber} is calling for a waiter`, confirmation: "A waiter has been notified and will be with you shortly!" },
    ...
    const handleSendOrder = () => {
        ...
        setMessages(prev => [...prev, {
            id: uid(),
            author: MessageAuthor.SYSTEM,
            text: `ð  ½ï¸ Your order has been sent to ${session.restaurantName}! The kitchen will prepare your items shortly.`,
        }]);
                    ...
                    <div className="bg-white w-full max-h-[88%] rounded-t-3xl overflow-hidden flex flex-col shadow-2xl">
                        ...
                        <div className="p-5 border-b border-cream-200 flex items-center justify-between bg-gradient-to-r from-stone-800 to-stone-700 flex-shrink-0">
                            <div>
                                ...
                                <p className="text-white/70 text-xs mt-0.5">{session.restaurantName} Â· Table {session.tableNumber}</p>
                            </div>
                        ...
                        <div className="p-4 border-t border-cream-200 bg-white flex-shrink-0 space-y-3">
                            ...
                            >
                                ...
                                {cartCount > 0 ? `Send Order Â· $${(cartTotal / 100).toFixed(2)}` : 'Add items to order'}
                            </button>
                        </div>
                    </div>
                </div>
